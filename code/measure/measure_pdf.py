...
#Measure the PDF of a kappa map
def pdf(maps,kappa_edges,indices):
	...
	#Allocate memory
	pdf_array = np.zeros((len(indices),len(kappa_edges)-1))
		
	#Measure the PDF
	for n,i in enumerate(indices):
		kappa,pdf_array[n] = maps[i].pdf(kappa_edges,norm=True)

	#Build the Ensemble
	columns = [ "k{0}".format(n) for n in range(1,len(kappa)+1) ]
	ensemble = Ensemble(pdf_array,columns=columns)
